...
class Path {
 public:
  ...
  /**
   * Returns PathType, the rule used to fill Path. PathType of a new Path is Path
   * FillType::Winding.
   */
  PathType getFillType() const;

  /**
   * Sets PathType, the rule used to fill Path.
   */
  void setFillType(PathType fillType);

  /**
   * Returns if PathType is InverseWinding or InverseEvenOdd.
   */
  bool isInverseFillType() const;

  /**
   * Replaces PathType with its inverse.
   */
  ...
  */
  ...
  bool isEmpty() const;
  /**
   * Returns true if the point (x, y) is contained by Path, taking into account PathType.
   */
   ...
   */
  void decompose(const PathIterator &interator, void *info = nullptr) const;
  /**
