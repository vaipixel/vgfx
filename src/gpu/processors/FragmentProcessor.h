...
class FragmentProcessor : public Processor {
 public:
  ...
  class Iterator {
   public:
    explicit Iterator(const FragmentProcessor *fragmentProcessor) {
      fpStack.push_back(fragmentProcessor);
    }

    explicit Iterator(const Pipline *pipline);
  ...
  class CoordTransformIterator {
   ...
   private:
    const FragmentProcessor *currentFragmentProcessor = nullptr;
    size_t currentIndex = 0;
    FragmentProcessor::Iterator fragmentProcessorIterator;
  };

  template<typename T, size_t (FragmentProcessor::*COUNT)() const>
  class BuilderInputProvider {
   public:
    BuilderInputProvider(const FragmentProcessor *fragmentProcessor, const T *t)
        : fragmentProcessor(fragmentProcessor), t(t) {
    }
  ...
  using TransformedCoordVars = BuilderInputProvider<ShaderVar, &FragmentProcessor::numCoordTransforms>;
  using TextureSamplers = BuilderInputProvider<SamplerHandle, &FragmentProcessor::numTextureSamplers>;
 ...
 protected:
  ...
  virtual void onSetData(UniformBuffer* buffer) const {
